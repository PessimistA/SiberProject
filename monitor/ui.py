...
class MonitorUI(ctk.CTk):
    def __init__(self, on_load_history=None, on_load_dates=None, on_load_ips=None, on_load_stats=None):
        super().__init__()
        self.title("ShadowGuard — Live Threat Monitor & Session History")
        ...
        self.lbl_status = ctk.CTkLabel(
            self.top_frame,
            text="🟢 ShadowGuard Active  |  UDP:5000 Listening  |  AI Analysis Engine Online",
    ...
    def _setup_live_tab(self):
        ...
        self.chat_frame = ctk.CTkScrollableFrame(
            self.tab_live,
            label_text="Live Network Traffic & Threat Stream",
    ...
    def populate_dates(self, dates):
        if not dates:
            ...
            # Also populate IPs for latest date
    ...
    def populate_stats(self, stats, attacker_db):
        ...
        if attacker_db:
            ...
            ctk.CTkLabel(atk_frame, text="🕵️  Known Attacker Profiles", font=("Arial", 13, "bold"), text_color="#ff4444").pack(anchor="w", padx=15, pady=(10, 5))
    ...
    def add_interaction_card(self, sender, text, role):
        ...
        if role == "attacker":
            ...
            border, header, tc = "#ff8800", f"[{timestamp}]  🚨  RULE ENGINE (IDS)", "#ffcc88"
        elif role == "web":
