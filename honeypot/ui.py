...
class HoneypotUI(ctk.CTk):
    def __init__(self, start_callback, stop_callback, port_info_callback, analyze_url_callback):
        super().__init__()
        self.title("Honeypot Control Center")
        ...
        self.tab_network = self.tabview.add("Network Defense")
        self.tab_web = self.tabview.add("Web Defense") # BAŞLIK GÜNCELLENDİ
    ...
    def setup_web_defense_tab(self):
        ctk.CTkLabel(self.tab_web, text="Simulated Static Web Analysis ", font=("Arial", 16, "bold"), text_color="#00aaff").pack(pady=(15, 0))
