...
class SecurityWebScanner:
    def __init__(self, ai_bridge_url="http://shadow_ai_koprusu:5000"):
        ...
        # Scope definition: Authorized local networks and file extensions
    ...
    def _is_url_in_scope(self, target_url):
        """Validates if the provided URL targets an authorized local/isolated network."""
    ...
    def _is_file_in_scope(self, file_path):
        """Validates if the uploaded file has a supported extension for static analysis."""
    ...
    def fetch_from_url(self, target_url):
        """Retrieves source code from an isolated local web server (Simulated Internet)."""
        if not self._is_url_in_scope(target_url):
            return None, "Target URL out of scope. Only local/isolated networks are permitted."
    ...
    def fetch_from_file(self, file_path):
        """Reads source code from an uploaded local file (Offline Payload Vault)."""
    ...
    def static_analysis(self, code):
        """Heuristic analysis utilizing Regex to identify known malicious patterns."""
        findings = []
    ...
    def ai_analysis(self, code):
        """Executes advanced behavioral analysis by delegating code segments to the AI Bridge."""
        # Enforcing token limits; extracting the first 2000 characters
        ...
        try:
            ...
            res = requests.post(f"{self.ai_bridge_url}/ai-sor", json=payload, timeout=30)
    ...
    def _run_analysis_pipeline(self, code, target_name):
        """Internal pipeline that processes the fetched code regardless of its origin (URL or File)."""
        ...
        # Phase 2: AI Behavioral Analysis
    ...
    def scan_file(self, file_path):
        """Public API: Scans an uploaded file."""
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if choice == '1':
            hedef = input("Enter Local URL (e.g., http://localhost/test.html): ")
